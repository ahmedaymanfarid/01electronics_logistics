...
AND branch_serial = " + branchSerial;
        ...
        public bool GetCustomsAgentsSerialsAndNames(ref List<AGENCY_MACROS.AGENCY_STRUCT> returnVector)
        {
            returnVector.Clear();

            String sqlQueryPart1 = "select agency_serial, customs_agency_name from customs_agency_name order by customs_agency_name;";
        ...
        public bool GetFullCustomsAgentInfo(int agentId, ref AGENCY_MACROS.AGENCY_FULL_INFO agency)
        {
            String sqlQuery = @"select customs_agency_name.customs_agency_name,[name]
from customs_agency_name,employees_info
where customs_agency_name.added_by = employees_info.employee_id
AND customs_agency_name.agency_serial = " + agentId;
            BASIC_STRUCTS.SQL_COLUMN_COUNT_STRUCT queryColumns = new BASIC_STRUCTS.SQL_COLUMN_COUNT_STRUCT();
            ...
            sqlQuery = @"select customs_agency_address.[address],district,cities.city,states_governorates.state_governorate,countries.country 
from customs_agency_address,districts,cities,states_governorates,countries
where districts.id = customs_agency_address.[address]
AND districts.city = cities.id 
AND cities.state_governorate = states_governorates.id
AND states_governorates.country = countries.id
AND customs_agency_address.agency_serial = " + agentId;
            ...
            sqlQuery = @"select telephone_number
from customs_agency_telephone,customs_agency_address
where customs_agency_address.address_serial = customs_agency_telephone.branch_serial
AND customs_agency_address.address_serial = " + agentId;
            ...
            sqlQuery = @"select fax
from customs_agency_fax,customs_agency_address
where customs_agency_address.address_serial = customs_agency_fax.branch_serial
AND customs_agency_address.address_serial = " + agentId;
        ...
        public bool GetAllContactsOfCustomsBranch(int branchSerial, List<AGENCY_MACROS.AGENCY_CONTACT_INFO> contacts)
        {
            String sqlQuery = @"select agency_contact_person_info.contact_id,employee_id,branch_serial,agency_contact_person_info.department,email,[name],gender,departments_type.department
from customs_agency_contact_person_info,departments_type
where customs_agency_contact_person_info.department = departments_type.id 
AND branch_serial = " + branchSerial;
            BASIC_STRUCTS.SQL_COLUMN_COUNT_STRUCT queryColumns = new BASIC_STRUCTS.SQL_COLUMN_COUNT_STRUCT();
            ...
            GetCustomsContactTelephones(contacts);

            return true;
        }

        private bool GetCustomsContactTelephones(List<AGENCY_MACROS.AGENCY_CONTACT_INFO> contacts)
        {
            for (int i = 0; i < contacts.Count; i++)
            {
                AGENCY_MACROS.AGENCY_CONTACT_INFO contact = contacts[i];
                ...
                String sqlQuery = @"select mobile from customs_agency_contact_person_mobile where branch_serial = " + contact.branchId + " AND contact_id = " + contact.contactId;
                BASIC_STRUCTS.SQL_COLUMN_COUNT_STRUCT queryColumns = new BASIC_STRUCTS.SQL_COLUMN_COUNT_STRUCT();
